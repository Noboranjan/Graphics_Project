...
void handleResize(int w, int h)
{
	...
	glClearColor(0.69, 0.34, 0.0, 0.0);//beside road
	glLoadIdentity();
...
bool collision()
{
    for(int i = 0; i<10; i++)
    {
        if(oppX[i] == cr_m_L_R && oppY[i] < cr_m_U_D + 0.18 && oppY[i] > cr_m_U_D - 0.18)
        {
                collide = 1;
                return true;
        }
...
//head
      ...
      glScalef(0.2,0.2,0.08);
      glutSolidCube(0.3);
      glPopMatrix();
//hand
      glPushMatrix();
      glTranslatef(-0.18, 0.03,0.64);
